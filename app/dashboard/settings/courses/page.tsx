...
type TabType = 'categories' | 'difficultyLevels'
...
export default function CoursesSettingsPage() {
  ...
  const [difficultyLevels, setDifficultyLevels] = useState<SettingsItem[]>([])
  ...
  const loadData = () => {
    setCategories(getSettings(STORAGE_KEYS.COURSE_CATEGORIES))
    setDifficultyLevels(getSettings(STORAGE_KEYS.COURSE_DIFFICULTY_LEVELS))
  }

  const getCurrentData = () => activeTab === 'categories' ? categories : difficultyLevels
  const getCurrentStorageKey = () => activeTab === 'categories' ? STORAGE_KEYS.COURSE_CATEGORIES : STORAGE_KEYS.COURSE_DIFFICULTY_LEVELS

  const getCurrentConfig = () => {
    return activeTab === 'categories' ? {
      title: 'Course Categories',
      description: 'Organize courses into learning categories',
      addButtonText: 'Add Category',
      showColor: true
    } : {
      title: 'Difficulty Levels',
      description: 'Define skill levels for course complexity',
  ...
  const tabs = [
    ...
    { id: 'difficultyLevels' as TabType, label: 'Difficulty Levels', count: difficultyLevels.length }
  ]
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">Courses Settings</h1>
            <p className="text-gray-400">Configure categories and difficulty levels</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-blue-500/10 border border-blue-500/20 p-4">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <p className="text-blue-300 font-medium">Learning Management Configuration</p>
            <p className="text-blue-400/80 text-sm mt-1">
              Organize courses by category and define difficulty levels to help learners find appropriate content.
            </p>
          </div>
        </div>
      </div>

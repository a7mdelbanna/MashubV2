...
export default function EditTransactionPage({ params }: { params: { id: string } }) {
    ...
    const fetchTransaction = async () => {
      ...
      try {
        ...
        const mockTransaction = {
          ...
          notes: 'Monthly rent payment for Cairo office space.',
        ...
        setOriginalState(mockTransaction.state)
        setIsPosted(mockTransaction.state === 'posted' || mockTransaction.state === 'void')
      } catch (error) {
  ...
  const handleChange = (field: string, value: string | boolean) => {
    setFormData(prev => {
      ...
      // Reset account when currency changes (only if not posted)
      if (field === 'currency' && !isPosted) {
        updated.accountId = ''
        updated.sourceAccountId = ''
        updated.destinationAccountId = ''
      }

      // Reset category when type changes (only if not posted)
      if (field === 'type' && !isPosted) {
        updated.categoryId = ''
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/finance/transactions/${params.id}`)
    } catch (error) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        <Link
          href={`/dashboard/finance/transactions/${params.id}`}
          ...
          <div className="mb-6 bg-red-500/10 border border-red-500/30 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-red-300">
                <p className="mb-2">
                  <strong className="text-red-400">This transaction cannot be edited:</strong> Posted and voided transactions are immutable.
                </p>
                <p>
                  If you need to correct this transaction, please void it and create a new one.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-gray-300">
                <p className="mb-2">
                  <strong className="text-white">Editing restrictions:</strong> You can edit this transaction because it's in "{originalState}" state.
                </p>
                <p>
                  Once posted, transactions cannot be edited. They can only be voided.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                } rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50`}
                placeholder="0.00"
                required
                disabled={isPosted}
              />
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50"
